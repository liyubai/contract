...
*/
	...
	$( "#dialogForm1" ).dialog({
		...
		buttons: {
			"确 定": function() {
				...
				var dicInfo = $("#vname");
				//取得字典类型下拉列表框对象
	            var dictypeid = $("#adictypeid");
				
               // alert(dicInfo + ", " + dictypeid);
				bValid = bValid && checkLength(dicInfo, "字典信息", 1, 100);
				bValid = bValid && checkLength(dictypeid, "字典类型", 1, 100);
                //alert(bValid);
				if ( bValid ) {

				    $("#cvname").val($("#vname").val());
					$("#cdictypeid").val($("#adictypeid").val());
	...
	$("#modalDialogButton1").click(function(){
		...
		$("#dialogForm1").dialog('option', 'title', '新建字典信息');
		$("#dicInfoForm").attr("action","createDicInfo.do");
	...
	$("#modalDialogButton2").click(function(){
		...
		if(count == 0){
			$("#dialogForm3Warning").html("<br/>请选择要删除的字典信息！"); //dialogForm3Warning：用于提示警告信息
			$( "#dialogForm3" ).dialog( "open" );
			return;
		}
		//进行相应的删除操作
		$("#dicInfoidsStr").val(ids);
    ...
    function execEdit(rid){
		...
		$.ajax({
			type:"post",
			url:"searchDicInfoByIdForEdit.do",
			data:params,
			dataType:"json",
			success:function(data){
				...
				$("#adictypeid").val(data.typeVId);
				
				$("#adictypeid").change(); 
                
				$("#dialogForm1").dialog('option', 'title', '编辑字典信息');
				$("#dicInfoForm").attr("action","updateDicInfo.do");
				$("#dialogForm1").dialog( "open" );
...
<s:if test="#r==null || #r==0">
	<button id="modalDialogButton1" class="green" primary="ui-icon-button-addnew">新建字典信息</button>
	
	<button class="normal barLeft" id="modalDialogButton2">删除字典信息</button></s:if>
	
	<button class="normal" id="modalDialogButton3">全部</button>
		
    <div class="buttonInput relative left" width="180" height="28">
     <input id="dicInfoName" name="dicInfoName"  type="text" maxlength="50" value="${key}"/><button id="modalDialogButton4" title="输入字典信息">查询</button>
    </div>
		...
		<table class="panelTable" id="usersTable" border="0" cellspacing="0" cellpadding="3">
			<thead>
			  ...
			  <tr>
				...
				<th class="thSortableGray">字典信息</th>
				<th class="thSortableGray">字典类型</th>
			...
			<s:iterator value="page.data">
			  <tr>
				...
				<td class="searchRange">${TDictype.VName}</td>
				<td class="searchRange">
				   <s:date name="DCreatedate" format="yyyy-MM-dd HH:mm:ss" />
				</td>
				<td>
				<s:if test="#r==null || #r==0">
				   <a href="javascript:execEdit('${VId}')" class="blue">修改</a></s:if>
...
<div id="dialogForm1"  title="新建字典信息">
	<p class="validateTips" >请填写所有信息</p>
    
	<fieldset>
    <span class="panelLine"></span>
		<label for="adictypeid">字典类型：</label>
        <div class="dropDownList">
        <button></button>
        <ul>
		     <li> 
              <s:iterator value="dicTypes" status="stu">
              	<a href="#N" val="${VId}" class="inline"  style="width:100px;" title="${VName}" ><s:if test="#stu.index==0"><span class="ui-icon ui-icon-bullet"></span></s:if>
                <s:set name="str" value="VName"></s:set>
                <s:if test="#str.length()>7">
                <s:property value="#str.substring(0,7)+'...'" />
                </s:if>
                <s:else>
                <s:property value="#str" />
                </s:else></a>
			  </s:iterator></li>
			   <s:iterator value="dicTypes" begin="0" end="0">
			     <input type="hidden"  id="adictypeid" value="${VId}" />
			   </s:iterator>
			  
        </ul>
		
    	</div><br/><span class="panelLine"></span>
    	<label for="vname">字典信息：</label>
...
<form id="dicInfoForm" method="post" action="createDicInfo.do">
	<input type="hidden" name="dic.VId" id="VId"/>
	<input type="hidden" name="dic.VName" id="cvname" />
	<input type="hidden" name="dic.TDictype.VId" id="cdictypeid" />
	<input type="hidden" name="key" value="${key}"/>
	<input type="hidden" name="pageIndex" value="${pageIndex}"/>
</form>

<form  action="searchDicInfos.do" method="post" id="searchForm">
  <input name="key" type="hidden" id="searchKey">
</form>
<form  action="deleteDicInfo.do" method="post" id="deleteForm">
    <input name="dicids" type="hidden" id="dicInfoidsStr">
    <input type="hidden" name="key" value="${key}"/>
    <input type="hidden" name="pageIndex" value="${pageIndex}"/>
</form>
