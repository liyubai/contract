...
$(function() {
	...
	$("div.dropDownList").find("button").css("width","85px").end().each(function(i){
		...
		else if(i==8){var d=new $.dropDownList(i,true,260);d.button.css("width", "80px");}
	...
	$("#baseButton").click(function(){
		...
		if(flag>0){
			$("#dialogForm3Warning").html("选择的学员中有已分配教员的，请重新选择。");
...
<button class="green"  id="baseButton">分配教员</button>
<button class="green"  id="changeButton">调换教员</button>
</div></s:if>
...
<div class="dropDownList barMiddle">
        ...
        <button>教员</button><span class="bar"></span>
        <ul>
            <li><a href="?year=${year}&classNo=${classNo}&isAssignid=1&xstuTypeid=${xstuTypeid}&xcompanyid=${xcompanyid}&xstatusid=${xstatusid}&xxjstatusid=${xxjstatusid}&xformid=${xformid}&xfreetypeid=${xfreetypeid}&name=${name}" val="1"  class="inline" style="width:50px;"><span class="ui-icon ui-icon-bullet"></span>教员</a> <s:iterator value="teacherList">
            <a href="?year=${year}&classNo=${classNo}&isAssignid=${VId}&xstuTypeid=${xstuTypeid}&xcompanyid=${xcompanyid}&xstatusid=${xstatusid}&xxjstatusid=${xxjstatusid}&xformid=${xformid}&xfreetypeid=${xfreetypeid}&name=${name}" val="${VId}" class="inline" style="width:50px;">${VName}</a>
           </s:iterator>
            <a href="?isAssignid=6"  val="6" class="inline"  style="width: 60px;">全部</a></li>
...
<div id="changeBaseDialog"  title="调换教员">
<fieldset><br><input type="text" name="cfocusn" style="width:0px; height:0px; border:0px;"/>
选择教员：
<span class="panelLine"></span>
<s:iterator value="teacherList">
<a href="#N" class="inline" style="width:70px; border:0; line-height:28px;"><input name="changeBaseName" type="radio" value="${VId}"><b></b>${VName}</a>
</s:iterator>
<span class="panelLine"></span>
调换日期： 
...
<div id="assignBaseDialog"  title="分配教员">
<fieldset>
...
选择教员：<br>
<s:iterator value="teacherList">
<a href="#N" class="inline" style="width:60px; border:0; line-height:28px;"><input name="baseName" type="radio" value="${VId}"><b></b>${VName}</a>
</s:iterator>
<span class="panelLine"></span>
...
<tr>
<td width="10%" height="90" align="left" valign="middle"><img src="themes/cupertino/images/warning.png" /></td>
<td width="90%" align="left" valign="middle"><span style="font-size: 14px; font-weight: bold; color: #555555; line-height:23px;">确认取消所选学员分配的教员吗？</span></td>
</tr>
</table>
</fieldset>
</div>
<s:include value="../student/advancedSearchDialog.jsp" />
<form action="saveStudentAssignTeacher.do" method="post" id="assignForm">
<input type="hidden"  name="assignTeacher.VId" id="VIds">
<input type="hidden"  name="assignTeacher.teacherBaseInfo.VId" id="orgid">
<input type="hidden"  name="assignTeacher.DAssigndate" id="DAssigntime">
</form>

<form action="saveStudentChangeTeacher.do" method="post" id="changeForm">
<input type="hidden"  name="changeTeacher.VId" id="cVIds">
<input type="hidden"  name="changeTeacher.teacherBaseInfoByVNewteacherid.VId" id="corgid">
<input type="hidden"  name="changeTeacher.DChangeddate" id="DChangedtime">
</form>

<form  action="escStudentAssignTeacher.do" method="post" id="escForm">
