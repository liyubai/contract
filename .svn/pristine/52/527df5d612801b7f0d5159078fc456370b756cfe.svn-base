...
$(function() {
	...
	$( "#selectOutlineDialogForm" ).dialog({
		...
		buttons: {
			"下一步": function() {
				...
				for(i=0;i<r1.length;i++){
					if(r1[i].checked==true){
						strid+=r1[i].value.split(",")[0] + ",";
						str+=r1[i].value.split(",")[1] + " ";
					}
				}
				if(str==""){
	...
	$( "#assignOutlineDialog" ).dialog({//输入分配大纲的内容
		...
		buttons: {
			"确 定": function() {
				...
				$("#DCreatedateid").val($("#enterDate").val());
				//进度
				var weekMax = $("#weekMax").val();
				var jd = "";
				for(var i=1;i<=weekMax;i++){
					jd+=$("#bweek"+i).val() + "," + $("#eweek"+i).val() + "," + $("#weekdate"+i).val() + ";";
					if($("#bweek"+i).val()=="" || $("#eweek"+i).val()=="" || $("#weekdate"+i).val()==""){
						alert("请输入完整的标准进度参数。");
						return;
					}
					if(!checkNum($("#bweek"+i).val()) || !checkNum($("#eweek"+i).val())){
						alert("周应为数字。");
						return;
					}
				}
	...
	$("img.addAssignOutline").click(function(){//为单独的学员分配大纲
		clearSelValue('checkboxName');
		execAssgin(this.id + ",");
		var r1=document.getElementsByName('checkboxName');
		for(i=0;i<r1.length;i++){
			if(r1[i].value.indexOf(this.id.split(";")[0])!=-1){
				r1[i].checked=false;
				r1[i].click();
				return;
			}
	...
	function execAssgin(optStr){//分配大纲
		...
		if(flag>0){
			$("#dialogForm3Warning").html("选择的学员中有已分配大纲的，请重新选择。");
			$( "#dialogForm3" ).dialog( "open" );
			return;	
		}
		if(str==""){
	...
	$("img.addWeek").click(function(){
		var weekMax = $("#weekMax").val();
		var weekContents = $("#weekContents").val();
		var id = weekMax;
		id=parseInt(weekMax) + 1;
		var content="从 <input id=\"bweek"+id+"\" type=\"text\" class=\"text ui-widget-content ui-corner-all\" style=\"width:20px;\"/> 到";
		content+=" <input id=\"eweek"+id+"\" type=\"text\" class=\"text ui-widget-content ui-corner-all\" style=\"width:20px;\"/>";
		content+=" 周的标准时间：<input id=\"weekdate"+id+"\" type=\"text\" class=\"text ui-widget-content ui-corner-all\"";
		content+=" style=\"width:40px;\" onMouseDown=\"deletebMF('weekdate"+id+"')\" onBlur=\"viewTime('weekdate"+id+"')\" />";
		content+=" <a href=\"javascript:void(0)\" onClick=\"viewDateDetail("+id+")\"><img src=\"themes/cupertino/images/detail.png\"";
		content+=" id=\""+id+"\" title=\"显示日期\" border=\"0\" align=\"absmiddle\"/></a>";
		content+=" <span id=\"cviewDate"+id+"\"></span><br/>";
		
		$("#weekMax").val(id);
		$("#weekContents").val(weekContents+content);
		$("#weekContent").html(weekContents+content);
	});

});

function viewDateDetail(id){
	
	var bweek = $("#bweek" +id).val();
	var eweek = $("#eweek" +id).val();
	var enterDate = $("#enterDate").val();
	if(bweek=="" || eweek=="" || enterDate==""){
		return;
	}
	if(!checkNum(bweek) || !checkNum(eweek)){
		alert("周应为数字。");
		return;
	}
	params = {"beignWeek":bweek,"endWeek":eweek,"selectDate":enterDate};
	$.ajax({
		type:"post",
		url:"showDateByWeekAndSelectDate.do",
		data:params,
		dataType:"json",
		success:function(data){
			
			$("#cviewDate"+ id).html(" 从 "+data.split(",")[0]+ " 到 " + data.split(",")[1]);
		},
		error:function(XMLHttpRequest, textStatus, errorThrown){
			if(XMLHttpRequest.getResponseHeader("sessionstatus")=="timeout"){ 
				return;
			}
			$("#dialogForm3Warning").html("数据库连接错误，获取信息失败，请重试。");
			$( "#dialogForm3" ).dialog( "open" );
			return;
		}
	});	
	...
	$.ajax({
		...
		success:function(data){
			...
			for(i=0;i<data.data.length;i++){
				...
				content+="<li><input name=\"radioName\" type=\"checkbox\" value=\""+data.data[i].VId+","+data.data[i].VName+"\"><A href=\"javascript:void(0)\" onclick=viewContent(\""+data.data[i].VId+"\") class=\"blue\">" + data.data[i].VName+"</a><span class=\"panelLine\"></span></li>";
			}
...
<div class="panelBox buttonBox" style="padding-bottom:10px; padding-left:3px;" ><button class="green" primary="ui-icon-button-addnew" id="assignOutlineButton">分配大纲</button></div></s:if>
...
<a href="?year=${year}&classNo=${VClassid}&xstuTypeid=${xstuTypeid}&xcompanyid=${xcompanyid}&xstatusid=${xstatusid}&xxjstatusid=${xxjstatusid}&xformid=${xformid}&xfreetypeid=${xfreetypeid}&isAssignid=${isAssignid}&name=${name}" val="${VClassid}">${VClassid}</a>
    ...
    <div class="dropDownList barMiddle">
        ...
        <ul>
            ...
            <li><a href="?year=${year}&classNo=${classNo}&isAssignid=2&xstuTypeid=${xstuTypeid}&xcompanyid=${xcompanyid}&xstatusid=${xstatusid}&xxjstatusid=${xxjstatusid}&xformid=${xformid}&xfreetypeid=${xfreetypeid}&name=${name}" val="2">未分配</a></li>
            <li><a href="?year=${year}&classNo=${classNo}&isAssignid=3&xstuTypeid=${xstuTypeid}&xcompanyid=${xcompanyid}&xstatusid=${xstatusid}&xxjstatusid=${xxjstatusid}&xformid=${xformid}&xfreetypeid=${xfreetypeid}&name=${name}" val="3">已分配</a></li>
...
<tr>
<td align="center"><input name="checkboxName" type="checkbox" value="${VId};${VCode};${VName};${flag}"  <s:if test='selIds!=null && selIds.indexOf(VId)!=-1'>checked="checked"</s:if>><b></b></td>
...
<td align="left">
<s:if test="#r==null || #r==0"><a href="javascript:void(0)"><img src="themes/cupertino/images/addfu.png"  id="${VId};${VCode};${VName};0" class="addAssignOutline" title="分配大纲" border="0" align="absmiddle"/></a><br></s:if>
<s:iterator value="stuClassList">
<s:if test="#r==null || #r==0"><a href="javascript:void(0)" ><img src="themes/cupertino/images/subfu.png" title="取消分配" class="escAssignOutline" border="0" align="absmiddle"  id="${VId}"/></a></s:if> <a href="javascript:void(0)" onClick="viewAssignDetail('${VId}')" class="blue">${className}</a><br/>
</s:iterator>
</td>
</tr>
</s:iterator>
...
<div id="escdialogForm">
  <fieldset>
    <table width="100%" border="0" align="center" cellpadding="0">
      <tr>
        <td width="10%" height="90" align="left" valign="middle"><img src="themes/cupertino/images/warning.png" /></td>
        <td width="90%" align="left" valign="middle"><span style="font-size: 14px; font-weight: bold; color: #555555; line-height:23px;">确认取消所选学员分配的大纲吗？</span></td>
      </tr>
    </table>
  </fieldset>
</div>
...
<div id="assignOutlineDialog"  title="分配大纲">
	...
	<span class="panelLine"></span>
	合&nbsp;同&nbsp;&nbsp;号： <input id="cVContact" type="text" class="text ui-widget-content ui-corner-all" style="width:200px;"/>
    <span class="panelLine"></span>
    ...
    <span class="panelLine"></span>
    入训时间：<input type="text" id="enterDate" class="Wdate ui-widget-content ui-corner-all text"  style="width:90px; border-color:#DDDDDD; height:16px;" onFocus="WdatePicker({cDCreateClassTime:'%y-%M-%d',dateFmt:'yyyy-MM-dd'})" value="${viewDate}" />
	<span class="panelLine"></span>
    标准进度：<a href="javascript:void(0)"><img src="themes/cupertino/images/addfu.png" class="addWeek" title="增加进度" border="0" align="absmiddle"/></a>
    <div style="padding-left:60px; line-height:30px;">从 <input id="bweek1" type="text" class="text ui-widget-content ui-corner-all" style="width:20px;"/> 到 <input id="eweek1" type="text" class="text ui-widget-content ui-corner-all" style="width:20px;"/> 周的标准时间：<input id="weekdate1" type="text" class="text ui-widget-content ui-corner-all" style="width:40px;" onMouseDown="deletebMF('weekdate1')" onBlur="viewTime('weekdate1')" />
    <a href="javascript:void(0)" onClick="viewDateDetail(1)"><img src="themes/cupertino/images/detail.png" title="显示日期"  border="0" align="absmiddle"/></a>
    <span id="cviewDate1"></span>
    </div>
    <div style="padding-left:60px;line-height:30px;" id="weekContent"></div>
...
<form  action="open_studentAssignOutlineList.do" method="post" id="advancedSearchForm">
...
<form  action="escAssignOutline.do" method="post" id="escForm">
<input type="hidden" name="id" id="escid">
</form>

<form  action="assignOutline.do" method="post" id="assignForm">
...
<input name="viewTrainsubject.DCreatedate" type="hidden" id="DCreatedateid">
<input name="viewTrainsubject.pdate" type="hidden" id="pdate">
</form>
<input type="hidden"  name="selIds" value="${selIds}" id="selSid">
<input type="hidden" id="selectStudentNameAndCode">
<%@ include file="../../result.jsp"%>
<input type="hidden" id="weekContents" value="">
<input type="hidden" id="weekMax" value="1">
</body>
</html>
<script>
