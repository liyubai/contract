...
(function ($) {
	...
	function test(){
		$("#accountStateStr").val($("#accountState").val());
		$("#paymentStateStr").val($("#paymentState").val());
		$("#partQueryAccount").submit();
		
/*			$.ajax({
				type:"post",
				cache : false,  
				url:"/contract/account/selectAccountById.do",
				data:{accountState:$("#accountState").val(),paymentState:$("#paymentState").val()},
				dataType:"json",
				success:function(data,statusCode ){
					alert(data);
					$("#accountData").html("");
					for(var i=0;i<data.length;i++){
						var tr = $("<tr class='data'><td><input name='checkboxName' type='checkbox' value=''><b></b></td></tr>");
						//$("#accountData").append("<tr class='data'><td><input name='checkboxName' type='checkbox' value=''><b></b></td></tr>");
						tr.append("<td>"+data[i].accountNo+"</td>");
						tr.append("<td>"+data[i].paymentNo+"</td>");
						tr.append("<td>"+data[i].contractNo+"</td>");
						tr.append("<td>"+data[i].fundsType+"</td>");
						tr.append("<td>"+data[i].fundsSubtotal+"</td>");
						if(data[i].accountState==2){
							tr.append("<td>已结算</td>");
						}else if(data[i].accountState==3){
							tr.append("<td>未结算</td>");
						}
						if(data[i].paymentState==2){
							tr.append("<td>已付款</td>");
						}else if(data[i].paymentState==3){
							tr.append("<td>未付款</td>");
						}
						tr.append("<td><a href='<%=request.getContextPath()%>/UpdateAccount/updateAccountInit.do?accountId="+data[i].accountNo+"&accountState="+data[i].paymentState+"' class='blue setStop'>修改</a></td>");
						$("#accountData").append(tr);
					};
				}, 
