...
*/
	...
	$("#modalDialogButton2").click(function(){
		...
		if(count == 0){
			$("#dialogForm3Warning").html("<br/>请选择要开通的其他人员信息！"); //dialogForm3Warning：用于提示警告信息
			$( "#dialogForm3" ).dialog( "open" );
			return;
		}
		//进行相应的开通操作
		$("#usersidsStr").val(ids);
		$( "#dialogForm2" ).dialog( "open" );  //dialogForm2：用于进行开通的提示警告信息
...
<s:else>
    ...
    <div class="buttonInput relative left" width="120" height="28">
	  ...
	  <s:if test="vcode==null || vcode == ''">
       <input id="tsovcode" name="tsovcode"  type="text" maxlength="50" value="其他人员编号" title="其他人员编号"/>  
	  </s:if>
	  <s:else>
	     <input id="tsovcode" name="tsovcode"  type="text" maxlength="50" value="${vcode}"   title="其他人员编号"/>  
	  </s:else> 
    ...
    <div class="buttonInput relative left" width="160" height="28">
	  ...
	  <s:if test="vname==null || vname == ''">
       <input id="tsovname" name="tsovname"  type="text" maxlength="50" value="编号/姓名" title="编号/姓名"/>  
	  </s:if>
	  <s:else>
	     <input id="tsovname" name="tsovname"  type="text" maxlength="50" value="${vname}"   title="编号/姓名"/>  
	  </s:else> 
		...
		<table class="panelTable" id="usersTable" border="0" cellspacing="0" cellpadding="3">
			<thead>
			  ...
			  <tr>
				...
				<th class="thSortableGray">人员编号</th>
				<th class="thSortableGray">人员姓名</th>
				<!--<th class="thSortableGray">身份证号</th>-->
				<th class="thSortableGray">出生日期</th>
				<th class="thSortableGray">联系方式</th>
			...
			<s:iterator value="page.data">
			  <tr>
				...
				<!--<td class="searchRange">${VSfz}</td>-->
				<td>	
				    <s:date name="DCsrq" format="yyyy-MM-dd" />		   
				</td>
				<td>
				   <!--
				    <a href="javascript:execEdit('${VId}')" class="blue">修改</a>&nbsp;&nbsp;<a href="javascript:recoverPass('${VId}')" class="blue">恢复默认密码</a>
				    -->
					${VLxfs}
				</td>
								<td>
				   <!--
				    <a href="javascript:execEdit('${VId}')" class="blue">修改</a>&nbsp;&nbsp;<a href="javascript:recoverPass('${VId}')" class="blue">恢复默认密码</a>
				    -->
					<s:if test="openStatus == 0">
					    <font color="#EB7979"><b>账号未开通</b></font>
					</s:if>
...
<div id="dialogForm2">
	<fieldset>
		...
		<div class="panelRight" style="margin-left: 75px;">
			<span style="font-size: 14px; font-weight: bold; color: #555555; line-height:23px;">确定要开通选中的人员账号吗？</span>
		</div>
	</fieldset>	
</div>
...
<form  action="searchOtherUsersInfoForOpenUserAccount.do" method="post" id="searchForm">
  <input name="vcode" type="hidden" id="searchVcode">
  <input name="vname" type="hidden" id="searchVanme">
  
</form>
<form  action="openOtherUsersUserAccount.do" method="post" id="deleteForm">
