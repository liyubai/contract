...
$(function() {
	...
	$("#auditUsed").click(function(){//审批生效
		var k = getSelValue('radioName');
		if(k==""){
			$("#dialogForm3Warning").html("请选择要审批的大纲。");
		...
		$("#outlineid").val(k);
		$("#VSubnameid").val("通过");
		$("#dialogFormAuditWarning").html("确定选中的大纲生效吗？");
	...
	$("#nopassbutton").click(function(){//不通过
		...
		$("#typeid").val(2);//不通过
		$("#outlineid").val(k);
		$("#newResonDialog").dialog('option', 'title', '大纲不通过的原因');
		$( "#newResonDialog" ).dialog( "open" );

	});
	$("#escbutton").click(function(){//取消
		var k = getSelValue('radioName');
		if(k==""){
			$("#dialogForm3Warning").html("请选择要审批的大纲。");
			$( "#dialogForm3" ).dialog( "open" );
		...
		$("#typeid").val(1);//取消
		$("#outlineid").val(k);
		$("#newResonDialog").dialog('option', 'title', '大纲取消的原因');
		$( "#newResonDialog" ).dialog( "open" );
	});
	...
	$( "#newResonDialog" ).dialog({//提交审批不通过输入原因界面 
		autoOpen: false,
		height: 300,
		width: 430,
		modal: true,
		...
		buttons: {
			"确 定": function() {
				$("#VSubnameid").val($("#cVSubnameid").val());

				if($.trim($("#VSubnameid").val())==""){
					alert("请输入原因。");
					return;
				}
				$( this ).dialog( "close" );
...
<s:if test="#r==null || #r==0"><button class="green" primary="ui-icon-button-confirm" id="auditUsed">审批生效</button></s:if>
    <button class="normal" id="nopassbutton">不通过</button>
	<button class="normal" id="escbutton">取 消</button>
    
    
    
 	<button class="normal barLeft" id="modalDialogButton9">未审批</button>
		...
		<table id="usersTable" class="colorTable" border="1" bordercolor="#E0E0E0" cellspacing="0" cellpadding="3">
            ...
            <s:iterator value="page.data" status="stat">
			  <tr>
                ...
                <td align="center">
                  <s:if test='IStatus==0'><font color="#50A13B">已生效</font></s:if>
                  <s:if test='IStatus==1'><font color="#CC9900">已取消</font></s:if>
                  <s:if test='IStatus==3'>未审批</s:if>
                  <s:if test='IStatus==2'><font color="#EB7979">未通过</font></s:if>
                </td>
				<td><div align="left"><A href="javascript:void(0)" onClick="viewDetail('${VId}','${VName}','${ILessons}','${STraintime}','${STrainningdevicetime}','${SPlanetime}','${ILessonLessons}','${SLessonTraintime}','${SLessonDTrainningdevicetime}','${SLessonDPlanetime}','${SMonijitime}','${SLessonMonijitime}')" class="blue">查看</A></div></td>
...
<div id="newResonDialog"  title="大纲取消的原因"> <br>
输入原因：&nbsp;<textarea  id="cVSubnameid" style="width:310px;height:150px;" class="text ui-widget-content ui-corner-all"></textarea>
</div>
...
<form  action="open_outlineAuditList.do" method="post" id="searchForm">
<input name="name" type="hidden" id="searchKey">
<input type="hidden" name="sortId" id="statusid">
</form>

<form  action="auditOutline.do" method="post" id="auditForm">
<input name="viewTrainsubject.typeid" type="hidden" id="typeid">
