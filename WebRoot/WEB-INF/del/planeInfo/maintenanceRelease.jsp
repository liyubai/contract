	...
	$("#modalDialogButton3").click(function(){
		...
		for(i=0;i<r1.length;i++){
			if(r1[i].checked==true){
				...
				str+=r1[i].value + ";" + shijian + ";" + times +",";
			}
		}

		if(k==0){
			$("#dialogForm4Span").html("请选择机务放行的飞机。");
...
<s:if test="#r==null || #r==0">
    <button id="modalDialogButton3" class="green">机务放行</button>
</s:if>
    ...
    <ul>
    <li><a href="open_maintenanceRelease.do?deviceType=${deviceType}&formid=&baseid=${baseid}"  val=""><span class="ui-icon ui-icon-bullet"></span>型号</a></li>
    <s:iterator value="planeformList">
    <li><a href="open_maintenanceRelease.do?deviceType=${deviceType}&formid=${VId}&baseid=${baseid}"  val="${VId}">${VName}</a></li>
    </s:iterator>
    </ul>
    ...
    <div class="dropDownList barMiddle">
        ...
        <ul>
        <li><a href="?deviceType=${deviceType}&baseid=&formid=${formid}" val=""><span class="ui-icon ui-icon-bullet"></span>所属基地</a></li>
        <s:iterator value="baselist">
        <li><a href="?deviceType=${deviceType}&baseid=${VId}&formid=${formid}"  val="${VId}">${VName}</a></li>
        </s:iterator>
        </ul>
...
<form  action="open_maintenanceRelease.do" method="post" id="searchForm">
<input name="deviceType" type="hidden" value="${deviceType}">
<input name="name" type="hidden" id="searchKey">
</form>

<form  action="saveMaintenanceRelease.do" method="post" id="repairResultForm">
<input name="deviceType" type="hidden" value="${deviceType}">
