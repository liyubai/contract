	...
	$("#modalDialogButton3").click(function(){
		...
		for(i=0;i<r1.length;i++){
			if(r1[i].checked==true){
				...
				if(!checkNum($("#shichang" + (i+1)).val().replace(":",""))){
					alert("飞行时间应为正整数。");
					$("#shichang" + (i+1)).focus();
					return;
				}
				if($("#kongzhong" + (i+1)).val()==""){
					alert("飞行时间不能为空。");
					$("#kongzhong" + (i+1)).focus();
					return;
				}
				if(!checkNum($("#kongzhong" + (i+1)).val().replace(":",""))){
					alert("飞行时间应为正整数。");
					$("#kongzhong" + (i+1)).focus();
					return;
				}
				...
				str+=r1[i].value.split(",")[0] + ";" +  $("#shijian" + (i+1)).val()+ ";" + $("#shichang" + (i+1)).val() + ";" 
				+ $("#times" + (i+1)).val() + ";" + $("#kongzhong" + (i+1)).val()  + ";" + itype +";" + reason + ",";
			}
		}
	...
	$( "#dialogForm1" ).dialog({
		...
		buttons: {
			"确 定": function() {
				...
				var flytime = $("#shichang" + btimeid).val();
				var kongzhong = $("#kongzhong" + btimeid).val();
				...
				if(!checkDouble(kongzhong.replace(":",""))){
					alert("飞行时间应为正整数。");
					$("#kongzhong" + btimeid).focus();
					return;
				}
				if(flytime==""){
					alert("飞行时间不能为空。");
				...
				if(!checkDouble(flytime.replace(":",""))){
					alert("飞行时间应为正整数。");
					$("#shichang" + btimeid).focus();
					return;
				}
				if(times==""){
					alert("起落架次不能为空。");
				...
				var vid = id.split(",")[0] + ";" + btime + ";" + t + ";" +flytime+";"+times + ";" + kongzhong;
				$("#oneId").val(vid);
				$("#toTeaForm").submit();
				$( this ).dialog( "close" );
...
<div class="Panel ui-corner-all-0" id="Panel2">
	...
	<div class="panelContent">
		...
		<table class="panelTable" id="usersTable" border="1" cellspacing="0" cellpadding="1">
			<thead>
			  <tr class="trSpliter">
				<th width="35"></th>
				<th width="80"></th>
            ...
            <s:iterator value="planeinfoList" status="i">
			  <tr class="normal">
                ...
                <td><input type="text" id="shichang${i.count}"  style="width:50px;" onMouseDown="deletebMF('shichang${i.count}')" onBlur="viewLongTime('shichang${i.count}')"/>
                <textarea name="textarea" id="textarea" style="width:50px;"></textarea></td>
