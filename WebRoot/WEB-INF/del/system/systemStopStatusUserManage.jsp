...
*/
	...
	$("#modalDialogButton4").click(function(){
		...
		$( "#dialogForm6" ).dialog( "open" );  //dialogForm2：于进行恢复的提示警告信息
...
<button id="modalDialogButton4" class="green" primary="ui-icon-button-addperson">恢复账号</button>
<button class="normal barLeft" id="modalDialogButton2">删除账号</button>
</s:if>
<button class="normal <s:if test="#r==null || #r==0">barRight</s:if>" id="modalDialogButton5">刷新</button>

    <div class="buttonInput relative left" width="160" height="28">
	
	  <s:if test="key==null || key == ''">
       <input id="userAccount" name="userAccount"  type="text" maxlength="50" value="用户账号" title="用户账号"/>  
	  </s:if>
	  <s:else>
	     <input id="userAccount" name="userAccount"  type="text" maxlength="50" value="${key}"   title="用户账号"/>  
	  </s:else> 
	  <button id="modalDialogButton6" title="输入用户账号">查询</button>
    </div>
</div>
		...
		<table class="panelTable" id="usersTable" border="0" cellspacing="0" cellpadding="3">
			<thead>
			  ...
			  <tr>
				...
				<th class="thSortableGray">是否为管理员</th>
			...
			<s:iterator value="page.data">
			  <tr>
				...
				<td class="searchRange" title="${VName}">${VUsername}</td>
...
<form  action="searchStopStatusUserAccount.do" method="post" id="searchForm">
  <input name="key" type="hidden" id="searchKey">
  
</form>
<form  action="deleteUserAccountForStopStatus.do" method="post" id="deleteForm">
...
<form  action="recoverUserAccountForStopStatus.do" method="post" id="recoverForm">
