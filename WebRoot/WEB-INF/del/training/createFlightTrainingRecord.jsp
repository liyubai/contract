...
$(function() {
	...
	$("div.dropDownList").find("button").css("width","120px").end().each(function(i){
		...
		if(i==0){var d=new $.dropDownList(i,true,260);d.button.css("width", "260px");}
		...
		else if(i==3){var d=new $.dropDownList(i,true,450);d.button.css("width", "100px");}
		...
		else if(i==7){var d=new $.dropDownList(i,true,280);d.button.css("width", "97px");}
	...
	$("#saveReocrdButton").click(function(){
		...
		*/
		...
		$("#sVFpos").val(bpos);//起飞站
		$("#sVLpos").val(epos);//到达站
		$("#sJtpos").val(jtpos);//经停站
		//var isThree = $("#isThree").val();
		var oneNum = $("#oneNum").val();
		...
		if(isThree==1 && (bpos==jtpos || bpos==epos || jtpos==epos)){
			alert("三点转场的课程，三站不能相同。");
			return;
		}
		//根据起飞站、经停站、到达站，计算单段里程数与总里程数
		if(isThree==1){
			var dt= 0;
			params = {"jtStation":jtpos,"flyStation":bpos,"endStation":epos};
			$.ajax({
				async:false,//同步请求
				type:"post",
				...
				success:function(data){
					...
					var one = data.IDh;//一段
					var second = data.IRootid;//二段
					if(oneNum>0 && (one<oneNum || second<oneNum)){
						alert("单段里程数不能小于"+oneNum+"千米");
						dt=1;
					}
					if(totalNum>0 && one+second<totalNum){
						alert("总里程数不能小于"+totalNum+"千米");
						dt=1;	
					}
				},
				error:function(){
...
function changeLesson(id){//根据课程id获取转场的始发站到达站
	...
	$("#splaneid").val("");
	$("#planeform").val("");
	if($("#slessonid2").val()==id){
		$("#viewContentByid").css('display','');
	}else{
		$("#viewContentByid").css('display','none');
	}	
	$("#cxlq").val("-1");
	$("#cxlq").change();
	$("#cJTpos").val("");
...
function getLessonContentData(id){//显示课目
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#oneNum").val(data.IOneNum);
			$("#totalNum").val(data.ITotalNum);
			var ids = $("#teacherids").val();
			if(data.IType=="飞机"){
				$("#itype").val(0);
			...
			content+="<td width=\"10%\">课目编号：</td><td d width=\"40%\">"+data.VNo+"</td>";
			content+="</tr>";
			...
			content+="<tr>";
			content+="<td height=\"24\" align=\"center\">版 本 &nbsp;号：</td><td>"+data.VVersion+"</td>";
			content+="<td >课目性质：</td><td>"+data.IKcxz+"</td>";
			content+="</tr>";
			...
			if(data.IIsthree==1){
				vthree+= "强制三点转场&nbsp;&nbsp;&nbsp;&nbsp;";
				if(data.IOneNum>0){
					vthree+= "单段里程数不能少于"+data.IOneNum+"千米&nbsp;&nbsp;&nbsp;&nbsp;";
				}
				if(data.ITotalNum>0){
					vthree+= "总里程数不能少于"+data.ITotalNum+"千米&nbsp;&nbsp;&nbsp;&nbsp;";
				}
			}else{
				vthree = "无";	
			}
			content+="<td  height=\"24\" colspan=\"3\" >"+vthree+"</td>";
			content+="</tr>"
		},
...
<div class="panelContent"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" >
  <tr>
    ...
    <td colspan="7"><div class="buttonBox"><div class="dropDownList barLeft"><button>大纲</button><span class="bar"></span>
    <ul>
    <s:iterator value="outlineList" status="statu">
    <li>
    <a href="#N" val="${VId}" title="${VName}"  onClick="changeOutLine('${VId}')">
    <s:if test='outlineid==VId'>
    ...
    <s:property value="#str" />
     ...
     <input type="hidden"  id="coutlineid" value="${outlineid}"/>
    </div>

<div class="dropDownList barMiddle"><button>单元</button><span class="bar"></span>
<ul><s:set name="ovalue" value="''"/><s:set name="unitids" value="''"/>
<s:iterator value="vunitList" status="statu"><s:set name="ovalue" value="#ovalue + 'ue' + #statu.count +VParentid+','"/><s:set name="unitids" value="#unitids+VId+','"/>
<li id="ue${statu.count}${VParentid}" class="<s:if test='outlineid==VParentid'>pview</s:if><s:else>phide</s:else>"><a href="#N"  val="${VId}" onClick="changeunit('${VId}')">
...
<input type="hidden"  id="selunitid" value="${uhid}"/>
      ...
      <div class="dropDownList barRight">
        ...
        <ul>
          <li><s:set name="lid" value="-1"/><s:set name="lvalue" value="''"/><s:set name="lessonids" value="''"/>
            <s:iterator value="phaseLessonList" status="statu"><s:set name="lvalue" value="#lvalue + 'le' + #statu.count +VParentid+','"/><s:set name="lessonids" value="#lessonids+VId+','"/><a href="#N" class="inline awidth" title="${VName}" style="<s:if test='#phid!=VParentid'>display:none</s:if>" id="le${statu.count}${VParentid}"  val="${VId}" onClick="changeLesson('${VId}')">
  ...
  <tr>
    ...
    <td><input type="text" id="cplaneid" style="width:85px; background-color:#E9E9E9; display:none"  class="text ui-widget-content ui-corner-all"   onKeyPress="return false" onpaste="return false"/>
    <div class="dropDownList" style="display:none" id="xqlid">
    <button>请选择</button>
    <ul><li>
    <a href="#N" val="-1"  class="inline" style="width:55px;"><span class="ui-icon ui-icon-bullet"></span>请选择</a>
    ...
    <input type="hidden"  id="cxlq" value="-1"/>
    </div></td>
  ...
  <tr>
    ...
    <button>起飞站</button>
    <ul><li>
    	<a href="#N" val="" class="inline" style="width:55px;"><span class="ui-icon ui-icon-bullet"></span>无</a>
        <s:iterator value="stationList" status="statu">
    ...
    <td colspan="8" height="10"></td>
		...
		<table id="usersTable" class="colorTable" border="1" bordercolor="#E0E0E0" cellspacing="0" cellpadding="2">
			...
			<tbody>
              ...
              <td align="center"><input type="text"  id="cILangnighttimes" class="text ui-widget-content ui-corner-all"  style="width:20px;" value="0" /></td>
				...
				<td align="center"><input type="text"  name="cflight" class="text ui-widget-content ui-corner-all"  style="width:50px;" /></td>
...
<div style="padding-left:10px; padding-right:10px; padding-top:10px;" id="viewContentByid"><s:set name="p" value="0"/>
...
<table id="usersTable3" class="colorTable" border="1" bordercolor="#E0E0E0" cellspacing="0" cellpadding="2" >
  ...
  <thead>
    <tr>
      ...
      <th rowspan="2">&nbsp;</th>
      <th rowspan="2">&nbsp;</th>
      <th rowspan="2"><div align="left">&nbsp;备注</div></th>
      </tr>
    <tr>
  ...
  <tbody>
    <tr>
      ...
      <td align="center">${VJtpos}</td>
      <td align="left">&nbsp;</td>
      <td align="left">&nbsp;</td>
      <td align="left">&nbsp;</td>
      <td align="left">&nbsp;<img src="themes/cupertino/images/detail.png" title="${VBz}" border="0"/></td>
      </tr>
  </tbody>
...
<input type="hidden" name="lessonid"  value="${lessonid}" id="slessonid2">
...
<input type="hidden" id="oneNum">
<input type="hidden" id="totalNum">

<input type="hidden"  id="statusMsgId" value="${statusMsg}" />
</body>
</html>
<script>
...
$(function(){
	if(operateResult !=null && operateResult!="" ){
		...
		if(operateResult=="1"){
			str= "训练记录保存失败";
			$("#resultid").html(str+" 预览...");
		}else if(operateResult=="0"){
			str= "训练记录保存成功";
			$("#resultid").html(str+" 预览...");
		}else{
			str= operateResult;
