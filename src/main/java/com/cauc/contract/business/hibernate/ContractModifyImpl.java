...
import com.cauc.contract.ui.core.JsonOpreat;
...
//	}
	...
	public void savePayMent(Contract oldConEneity, String gradeInfo, String payMentData)
		...
		// 合同表年级的json数据	   
		List<ContractGradeInfo> gradeInfoJsonList = JsonOpreat.makeGradeInfo(gradeInfo, ContractGradeInfo.class);
		// 合同表的json数据
			...
			try {
				...
				for (int i = 0; i < gradeInfoJsonList.size(); i++) {
					...
					for (int j = 0; j < payMentJsonList.size(); j++) {
						...
						addPayMent.setPGeneralPhase(payMentJsonList.size()); //当前年级总阶段
						...
						//学生信息
						String tempStr = gradeInfoJsonList.get(i).getStudentNameNo().toString();
						tempStr = tempStr.replace("[", "").replace("]", "");
						addPayMent.setPStudentInfo(tempStr);
						// 学生人数
...
//			     + "C_COMPY,C_SIGN_TIME,C_PRIVATE_NAME from Contract  where  C_CONTRACT_ID = "+con.getCContractId();
	...
	public String searchStudent(int gradeyear,String contractNo) throws FinanceServiceException {
		...
		/*String sql="  select No, StudentName from Student where Grade = "+gradeyear+""
				  + " order by No" ;*/
		String sql = " select No, StudentName,contractNo from Student where Grade = "+gradeyear+" "
			       + " and ( contractNo is null or contractNo='' or contractNo='"+contractNo+"' )"
				   + " order by No" ;
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED)
	public void updateStudent(String contractId, String student) throws FinanceServiceException {
		...
		stu= stu.substring(0,stu.length()-1);
			String sql ="update student set contractNo='"+contractId+"' where No  in ("+stu+")";
			super.createSQLQuery(sql).executeUpdate();
